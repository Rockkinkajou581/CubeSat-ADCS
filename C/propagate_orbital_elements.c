...
void propagateOrbitalElements(float semi_major_axis, float eccentricity, float inclination, float ascending_node, float periapsis, float true_anomaly, float time_delta, float *array) {
...
void testKepler(float axis, float e0, float inc0, float RAAN0, float argp0, float nu0) {
    ...
    for (int j =0; j < 120; ++j) {
        float array[6];
        propagateOrbitalElements(axis, e0, inc0, RAAN0, argp0, nu0, j * 60, array);
